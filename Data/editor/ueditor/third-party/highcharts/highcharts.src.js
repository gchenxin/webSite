...
Axis.prototype = {
	...
	update: function (newOptions, redraw) {
		...
		newOptions = chart.options[this.xOrY + 'Axis'][this.options.Index_index] = merge(this.userOptions, newOptions);
	...
	remove: function (redraw) {
		...
		chart.options[key].splice(this.options.Index_index, 1);
		...
		each(chartSeries, function (series) {
			...
			if (isString(linkedTo)) {
				if (linkedTo === ':previous') {
					linkedTo = chart.series[series.Index_index - 1];
				} else {
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		stableSort(chartSeries, function (a, b) {
			return pick(a.options.Index_index, a._i) - pick(b.options.Index_index, a._i);
		});
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.Index_index) ||
							(seriesOptions[AXIS] !== UNDEFINED && seriesOptions[AXIS] === axisOptions.id) ||
							(seriesOptions[AXIS] === UNDEFINED && axisOptions.Index_index === 0)) {
	...
	setStackedPoints: function () {
		...
		for (i = 0; i < yDataLength; i++) {
			...
			stack = stacks[key][x];
			stack.points[series.Index_index] = [stack.cum || 0];
			...
			stack.points[series.Index_index].push(stack.cum);
	...
	setPercentStacks: function () {
		...
		each([stackKey, '-' + stackKey], function (key) {
			...
			while (i--) {
				...
				pointExtremes = stack && stack.points[series.Index_index];
				if (pointExtremes) {
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			if (stacking && series.visible && stack && stack[xValue]) {
				...
				stackValues = pointStack.points[series.Index_index];
