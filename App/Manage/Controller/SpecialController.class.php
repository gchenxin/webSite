 ...
 * @Last Modified time: 2017-10-18 17:40:51
 */
...
class SpecialController extends CommonController {
	...
	public function index() {
		...
		$where   = array('special.delete_status' => 0);
		...
		$art   = D('SpecialView')->nofield('content')->where($where)->order('point DESC, id DESC')->limit($limit)->select();
	...
	public function addPost() {
		...
		if ($id = M('special')->add($data)) {
			...
			$content   = '';
			insert_att_index($content, $first_pic, $id, 0, 'special');
	...
	public function trach() {
		...
		$where = array('special.delete_status' => 1);
		$count = D('SpecialView')->where($where)->count();
	...
	public function del() {
		...
		if (false !== M('special')->where(array('id' => $id))->setField('delete_status', 1)) {
	...
	public function delBatch() {
		...
		if (false !== M('special')->where(array('id' => array('in', $idArr)))->setField('delete_status', 1)) {
	...
	public function restore() {
		...
		if (false !== M('special')->where(array('id' => $id))->setField('delete_status', 0)) {
	...
	public function restoreBatch() {
		...
		if (false !== M('special')->where(array('id' => array('in', $idArr)))->setField('delete_status', 0)) {
	...
	public function clear() {
		...
		if (M('special')->delete($id)) {
			...
			M('AttachmentIndex')->where(array('arcid' => $id, 'model_id' => 0, 'desc' => 'special'))->delete();
	...
	public function clearBatch() {
		...
		if (M('special')->where($where)->delete()) {
			// delete picture index
			M('AttachmentIndex')->where(array('arcid' => array('in', $idArr), 'model_id' => 0, 'desc' => 'special'))->delete();
			$this->success('彻底删除成功', U('Special/trach'));
		} else {
			$this->error('彻底删除失败');
		}
