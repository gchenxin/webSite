...
<head>
	...
	<script language="JavaScript">
	    ...
	    var SELF='/manage.php?s=/System/site';
	...
	<div class="row margin-botton">
        <div class="col-md-12 text-right">
            <div class="btn-group btn-group-md">
                 <button class="btn btn-default" type="button" onclick="goUrl('<?php echo U('Index_index');?>')"><em class="glyphicon glyphicon-th-list"></em> 配置项管理</button>
            </div>
        </div>
    </div>
					...
					<div class="tab-content" style="margin-top:20px;">
						<?php if(is_array($vlist)): foreach($vlist as $key=>$v): ?><div role="tabpanel" <?php if($key == 1): ?>class="tab-pane active"<?php else: ?>class="tab-pane"<?php endif; ?> id="tabConent<?php echo ($key); ?>">
							<?php if(is_array($v)): $i = 0; $__LIST__ = $v;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><div class="form-group">
								...
								<div class="col-sm-2"><span class="icon-question-sign" data-toggle="tooltip" data-placement="left" title="<?php echo ($config["remark"]); ?>"></span></div>
